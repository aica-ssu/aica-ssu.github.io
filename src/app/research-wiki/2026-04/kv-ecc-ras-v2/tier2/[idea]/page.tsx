...
export const metadata: Metadata = {
  title: "Tier-2 Idea — KV Cache ECC + RAS v2 - AICA Lab",
  robots: { index: false, follow: false },
};

const ROUTE_BASE_DIR = "/research-wiki/2026-04/kv-ecc-ras-v2/tier2";

export async function generateStaticParams() {
  return [
    { idea: "01-ecs-trace" },
    { idea: "02-quarantine-mini" },
    { idea: "03-prefixguard-lite" },
  ];
}
...
export default async function Tier2IdeaPage({ params }: { params: Promise<{ idea: string }> }) {
  ...
  const filePath = path.join(
    process.cwd(),
    `src/data/docs/research-wiki/2026-04-kv-ecc-ras-v2/tier2/${idea}.md`
  );
