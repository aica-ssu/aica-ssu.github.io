...
const recentSessions: SessionCard[] = [
  {
    ...
    keywords: [
      "NVIDIA Cosmos3 omnimodal world model (language/image/video/audio/action)",
      "Mixture-of-Transformers dual-tower (Reasoner=AR + Generator=diffusion, attention만 공유)",
      "16B / 활성 8B — phase 별 한 tower만 활성, RTX Pro 6000 ×2 → 단일 edge GPU",
      "Jetson Orin / Thor edge serving (UMA / LPDDR / thermal)",
      "static conditioning KV (denoise 전 기간 read-only K_AR/V_AR)",
      "flow-step×layer 누적 denoising error bound (N×L closed-form)",
      "tower-asymmetric PTQ — co-init quant-sensitivity divergence ρ_ℓ",
      "phase-deterministic weight residency (double-buffered cudaMemcpyAsync)",
      "intra-request AR↔DM 분리 DVFS (GPU/EMC) + J/chunk ledger",
      "L2 set-aside pinning of static cross-tower K_AR",
      "VAE placement LUT (GPU/DLA/PVA accelerator-complex)",
      "vLLM-Omni (vllm-omni@95d56cf) 단일-device serving runtime",
      "R72 첫 적용 — 전 문서 source-code anchored (anchor 검증 47/48)",
      "Q2×L3 paper-pair (INT4 K_AR 가 L2 3MB set-aside fit 전제)",
    ],
    summary:
      "2026-06-04 Cosmos3 세션(`cosmos3-mot-edge-serving`)의 **DEPTH RE-RUN** — 사용자 피드백(\"구현 디테일이 너무 얇다\")에 따라 신설된 **Rule R72 의 첫 적용 사례**. Top-6 idea 와 점수는 직전 세션 계승, 6개 tier 문서를 **local repo 클론 source-code anchoring** 으로 전면 재작성.\n• MoT dual-tower (Reasoner AR + Generator diffusion, attention만 공유) 16B/활성 8B 를 RTX Pro 6000 ×2 요구(§4.2.5)에서 **단일 edge GPU** 로 서빙하는 6 idea — Tier-1 {Q2 ANCHOR 7.6, Q1 DRIFT 7.4, S1 TIDELOOM 7.4} + Tier-2 독립 {S2 DUOCLOCK 7.4, L3 KEELKV 6.6, S4 SIDEPOOL 6.2}.\n• Tier-1 핵심: 정적 K_AR 1-shot Hadamard quant + flow-step×layer N×L 누적 bound (ANCHOR) / tower 민감도 발산 ρ_ℓ 의 co-init controlled-attribution PTQ (DRIFT) / phase-100%-결정론 weight residency + 단일-device MoT runtime (TIDELOOM).\n• **R72 첫 적용**: 전 문서 source-code anchored (`vllm-omni@95d56cf`), Phase-2 adversarial reviewer co-sign — anchor 정확도 **47/48**, 5 fix-now 정정 반영, code-수준 신규 발견 10건 확정.\n• pair {Q2 × L3}: policy 현실 K_AR(BF16 ~4.3MB/layer) > 3MB L2 set-aside → **L3 per-layer pin 이 Q2 INT4(CFG ON 2벌 = 2.16MB ≤ 3MB fit)를 사실상 전제**.\n• 전 세션 최강 경쟁자 [arXiv:2604.27792](https://arxiv.org/abs/2604.27792) **MotuBrain** (three-stream MoT world-action ~70% overlap) — 단 edge UMA/offload/DVFS/L2 미다룸이 유일한 숨구멍 → 전 idea 의 차별화 anchor.",
    detailed: true,
    tierTable: [
      { tier: "🥇 Tier-1", idea: "ANCHOR (Static-KV 1-shot Quant + Flow-Step×Layer N-step Denoising Error Bound)",
        venue: "MLSys 2027 / ICML 2027", score: "8/8/8 → 7.6",
        href: "/research-wiki/2026-06/cosmos3-edge-serving-deep/tier1/01-anchor" },
      { tier: "🥈 Tier-1", idea: "DRIFT (Tower-Asymmetric PTQ via Co-Init Quant-Sensitivity Divergence ρ_ℓ)",
        venue: "NeurIPS 2027 / ICML 2027 / MLSys 2027", score: "7/8/8 → 7.4",
        href: "/research-wiki/2026-06/cosmos3-edge-serving-deep/tier1/02-drift" },
      { tier: "🥉 Tier-1", idea: "TIDELOOM (Single-Device MoT Runtime + Phase-Deterministic Tower Residency)",
        venue: "MLSys 2027 / ASPLOS 2027", score: "7.4 (★impact 8 / ▼nov 7)",
        href: "/research-wiki/2026-06/cosmos3-edge-serving-deep/tier1/03-tideloom" },
      { tier: "T1", idea: "DUOCLOCK (Intra-Request AR↔DM EMC/GPU 분리 DVFS + J/chunk Ledger)",
        venue: "DATE 2027 / ISLPED 2027 / IISWC 2027", score: "6/7/8 → 7.4",
        href: "/research-wiki/2026-06/cosmos3-edge-serving-deep/tier2/01-duoclock" },
      { tier: "T2", idea: "KEELKV (Static Cross-Tower K_AR Per-Layer L2 Set-Aside Pinning, Q2 paper-pair)",
        venue: "DAC 2027 / DATE 2027", score: "6.6 (★ai-impl 8 / ▼nov 6)",
        href: "/research-wiki/2026-06/cosmos3-edge-serving-deep/tier2/02-keelkv" },
      { tier: "T3", idea: "SIDEPOOL (Frozen-Tokenizer Accelerator-Complex Placement LUT + Async Overlap)",
        venue: "DATE 2027 / ISPASS 2027 / DAC 2027", score: "6/6/6 → 6.2",
        href: "/research-wiki/2026-06/cosmos3-edge-serving-deep/tier2/03-sidepool" },
    ],
    extraLinks: [
      { label: "📊 Landing (Decision Tree + Essential Reading + Deep-Dive 신규 발견 10건)",
        href: "/research-wiki/2026-06/cosmos3-edge-serving-deep" },
      { label: "📜 미선정 9건 (06-04 계승, drop 3 + 흡수 6 — RELAY / LEDGERMARK / Cascade / Herald / Switchback 등)",
        href: "/research-wiki/2026-06/cosmos3-edge-serving-deep/unselected" },
    ],
  },
  {
    date: "2026-06-04",
