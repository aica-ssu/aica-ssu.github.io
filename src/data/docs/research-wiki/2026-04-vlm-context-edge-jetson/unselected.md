...
본 세션 21 idea (3 expert × 7) 중 Phase 2 (novelty / diff / impact 3-axis review + scoop-similarity critique) 후 Phase 1'' 최종 선정 사항:
- **Tier-1 Top 3 채택**: [CacheVeil](/research-wiki/2026-04/vlm-context-edge-jetson/tier1/01-cacheveil.md) (★ lead) / [SHOAL](/research-wiki/2026-04/vlm-context-edge-jetson/tier1/02-shoal.md) / [VESPER](/research-wiki/2026-04/vlm-context-edge-jetson/tier1/03-vesper.md)
- **Tier-2 독립 Top 3 채택**: [TUFA](/research-wiki/2026-04/vlm-context-edge-jetson/tier2/01-tufa.md) / [ShelfSwap](/research-wiki/2026-04/vlm-context-edge-jetson/tier2/02-shelfswap.md) / [Glacier Migrate](/research-wiki/2026-04/vlm-context-edge-jetson/tier2/03-glacier-migrate.md)
- **미선정 15편**: 본 파일.
...
| 7 | **DualLane** (Dual-NVDLA + GPU 3-way + SLC partition) | legacy-sys | MICRO / HPCA | 7.0 / 8.0 / 8.6 | **Nova [arXiv:2509.21301](https://arxiv.org/abs/2509.21301) 50-70% CONCURRENT** scoop, repositioning 부담 큼. SHOAL 과 DLA axis 공유 — Phase 1' 에서 SHOAL 이 더 distinctive (layer-time KV residence vs spatial-time DLA layer-split). | DualLane 의 4-mechanism (Dual-DLA + GPU + SLC + zero-copy) 분리 측정 ablation 시 Tier-1 재진입 가능 — 본 bundle 은 Phase 1' 단계에서 drop |
| 8 | **CacheVeil** | legacy-sys | (선정됨 — Tier-1 #1) | 8.5 / 7.0 / 8.5 | — | — |
...
| 17 | **Glacier Migrate** | hwpim | (선정됨 — Tier-2 #3) | 6.0 / 7.0 / 7.0 | — | — |

> **참고**: 위 표의 "#" 컬럼은 본 unselected.md 의 행 번호가 아닌 21 idea 전체 enumeration 에서의 원안 번호. 실제 미선정 idea 는 15편 (8/CacheVeil + 17/Glacier Migrate 는 채택).
...
| 채택 | Phase 2 평균 | 선정 사유 |
|------|-------------|----------|
| ★ CacheVeil (Tier-1 lead) | 8.00 | 최고 novelty 8.5, ARM CMN register-level partition + visual KV pin + CPU isolation. Edge SLC scope first-to-report. JetPack 7.1 fixed-target + sysfs fallback 명시 후 진입 |
| SHOAL (Tier-1) | 8.07 | 최고 diff 8.5, DLA → KV residence enum layer-단위 dynamic. vLLM PagedAttention block table 직접 patch 가능. FastVLM/V2Drop 추가 baseline 후 진입 |
| VESPER (Tier-1) | 8.10 | 최고 impact 8.8, UMA dual-view + NEON SIMD CPU pruner. system gain vs algorithmic gain 분리 ablation 강화 후 진입 |
| TUFA (Tier-2) | 6.47 | Orin Nano-specific 7W constraint first-to-report. trivialty 회피 명시 |
| ShelfSwap (Tier-2) | 6.07 | thermal-driven UMA zone migration first-to-report. SparseDVFS / Four Over Six adjacent baseline 명시 |
| Glacier Migrate (Tier-2) | 6.67 | DLA SRAM 가상 PIM emulation의 pixel-level workload-specific axis. 사용자 지시 "PIM 억지 적용 금지" 준수 |
