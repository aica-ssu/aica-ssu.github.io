...
- **RQ-A.4 (cross-share)**: ATRIUM 의 LayerClassifier (visual_attn_ratio HOT/COLD/MEDIUM 분류) 를 Prism M4 의 prerequisite 신호로 cross-share 시, sequential development path (ATRIUM W1-12 prototype → Prism 추가 W13-24) 가 학생 1명 12-16주 budget 안에 fit 가능한가?
...
2. **Qwen3-VL Tech Report** — Alibaba, [arXiv:2511.21631](https://arxiv.org/abs/2511.21631) — DeepStack architecture
   ...
   - **분담**: Prism (DeepStack-aware NVFP4 anchor), Bivouac (visual KV cluster), Harbinger (visual entropy signal)
...
3. **NVFP4 NVIDIA Tech Blog** — [NVIDIA NVFP4 Tech Blog](https://developer.nvidia.com/blog/introducing-nvfp4-for-efficient-and-accurate-low-precision-inference/)
   - **읽어야 하는 이유**: Blackwell native NVFP4 (E2M1, 16-element block scaling, FP8 micro-block scale + FP32 second-level scale) format. 4-bit 4:8 sparsity 와의 결합. Prism / Obelisk 의 baseline format
   - **분담**: Prism, Obelisk

4. **VL-Cache** — Tu et al., [arXiv:2410.23317](https://arxiv.org/abs/2410.23317), **ICLR 2025**
   - **읽어야 하는 이유**: VLM KV cache eviction 의 published baseline. layer-adaptive sparsity budget allocation. 10% memory budget 으로 full cache 동등 정확도 + 7.08× decode speedup. Bivouac 의 closest competitor
   - **분담**: Bivouac, RadixVL, Strata

5. **Sarathi-Serve** — Agrawal et al., [USENIX OSDI 2024](https://www.usenix.org/conference/osdi24/presentation/agrawal)
   - **읽어야 하는 이유**: Continuous batching + chunked prefill 의 published baseline. Mistral-7B 단일 A100 2.6× serving capacity. Strata / RadixVL / Obelisk 의 system baseline
   - **분담**: Strata, RadixVL, Obelisk
...
|-----|----------------|----------------|
| GAP-1 (Layer-uniform precision) | 🥇 Prism (M1 DALMP / M2 4:8 sparsity / M3 KSF-VCD / M4 LayerClassifier) + 4️⃣ ATRIUM (M1 LayerClassifier — visual_attn_ratio HOT/COLD/MEDIUM cross-share) | T3 Obelisk (per-stage power cap + NVFP4 + MoE expert placement) |
| GAP-2 (KV cache layer-uniform) | 🥈 Bivouac (M1 HSCV HOT-only cluster / M2 CFCR cosine cross-frame / M3 LACB layer-adaptive k) + 4️⃣ ATRIUM (M3 DeepStack L0-3 alloc skip — visual KV memory eff.) | T1 Strata (M1 LSS / M2 3TMM-AVT) |
| GAP-3 (Power-envelope blind) | 🥇 Prism (M3 visual-context DVFS slip) | T2 Harbinger (M3 PEAFL power-envelope locking) |
| GAP-4 (Phase-uniform caching) | 🥉 RadixVL (M1 phase-aware LSH 16/8/0) | — |
...
| GAP-5 (Static KV layout) | — | T1 Strata (M1-M3 stratified + page-color UMA bank micro-tier) |
| GAP-6 (Single-GPU large MoE) | — | T3 Obelisk (M1 SPVL Green Context / M2 MERL expert placement / M3 DPCS) |
...
└─────────────────────────────────────────────────────────────────────────┘
        ...
        │      │                                                                       Prism M4 enabler 로만 활용
        │      └─ Outperform (decode +18%↑ + L2 hit +30%pp) ──→ HPCA 2027 retain +
        │                                                       Prism 와 sequential development path 권장
        │
        ├─ T1 Strata (RTX 5090 + Jetson Thor)
        ...
        │
        ├─ T2 Harbinger (Jetson Orin NX 16GB)
...
│  - Inter-idea dependency 정리: Prism 의 LayerClassifier (M4)         │
│    가 Bivouac / Strata / Harbinger 의 layer-class      │
│    입력으로 cross-share                                                     │
└─────────────────────────────────────────────────────────────────────────┘
```
...
|------|----------------|---------------------|-------------------------------------------|------------------------|
| 🥇 Prism | W8-12 full eval Jetson Thor + RTX 5090 secondary, MLSys submission | Tier-2 강등, M2 4:8 sparsity drop, 3-mech (M1+M3+M4) 로 축소 | M3 visual-context DVFS slip drop, M1 DALMP + M4 LayerClassifier 만 retain (FGMP/MicroMix 와 head-to-head 만) | Tier-1 retain + standalone variant Tier-2 spinoff (KSF-VCD 단독으로 ISLPED) |
| 🥈 Bivouac | W7-8 Mosaic reproduction 후 차별 5 axis (HOT-only / edge Jetson / 1-bit mask + 4-bit delta / RAFT-cosine cost / layer-adaptive k) ablation NeurIPS submission | Mosaic 차별 axis 5개 중 ≤ 3 verified → Tier-2 강등 (ICML workshop) | Mosaic > 65% overlap 추가 발견 → drop 또는 M1+M3 만 retain (CFCR drop, single-shot only) | Mosaic 대비 +20% 우위 → Tier-1 retain + ICML 2026 spinoff (cluster centroid theory) |
| 🥉 RadixVL | RQ-3.4 W12 검증 시 vs VLCache 추가 -10%↑ → Tier-1 retain OSDI/SOSP | vs VLCache -5~-10% → Tier-1 borderline MLSys poster, RA-2VPB 만 강조 | vs VLCache -5% 미만 → Tier-2 강등 EuroSys/FAST, Phase-aware policy 단독 contribution | vs VLCache -20%↑ → Tier-1 retain + workshop on similarity caching |
| T1 Strata | RQ-4.3 page-color delta 60-120ns 측정 → effective bandwidth +15% 달성 → ASPLOS/DAC submission | RQ-4.3 page-color delta < 60ns → M3 PCA-UBP drop, M1+M2 (3-tier physical + L2 anchor pin) 만 retain | UMA 4-tier collapse → drop, CARILLON 흡수 시도 (이미 W3 후보 미선정 idea) | Anchor visual L2 pin gain > 20% → Tier-1 격상 시도, KVQuant 의 spatial pinning generalization 으로 NeurIPS spinoff |
| T2 Harbinger | Orin NX 25W envelope decode +32% / Energy -28% / peak -18% 달성 → DATE/ISLPED submission | visually ambiguous task accuracy drop > 3pt → M2 LCEH drop, M1+M3 (speculative + DVFS) 만 retain → IEEE CAL | nvpmodel sub-ms switching failure rate > 5% → M3 PEAFL drop, M1+M2 만 EMNLP findings | Peak power -25%↑ → Tier-1 격상 시도 ISLPED 풀논문 |
| T3 Obelisk | RTX 5090 + Qwen3-VL-30B-A3B decode +24% / power -13% 달성 → MLSys/DAC submission | DynaExq 추가 차별 axis (Green Context vision/LLM 분할 + per-stage power cap) 입증 실패 → M2 MERL drop, M1+M3 만 retain | 575W TDP thermal runaway 발생 → M3 DPCS 보강 (vision/decode 만 cap, prefill 무관) | +30% throughput → Tier-1 격상 + MLSys 풀논문 진입, MoE-on-consumer-GPU 단독 axis |

## 5. Tier-1 Top 4 (요약 + contribution bullet R15-β) — 2026-04-28 ATRIUM retain 포함

| Rank | Title | Score | 5-axis | 링크 |
|------|-------|-------|--------|------|
| 🥇 | Prism | nov 7.5 / diff 7.5 / impact 9.0 | Performance +18-25% / Energy -22% / Memory -32% / Power -9% | [tier1/01-prism.md](tier1/01-prism.md) |
| 🥈 | Bivouac | nov 6.5 / diff 8.0 / impact 8.0 | decode +35% / KV memory -65% / Energy -18% | [tier1/02-bivouac.md](tier1/02-bivouac.md) |
| 🥉 | RadixVL | nov 6.0 / diff 8.5 / impact 9.0 | TTFT -22-30% / KV -30-45% / Energy -15-20% | [tier1/03-radixvl.md](tier1/03-radixvl.md) |
...
### 5.1 🥇 Prism — contribution bullet (R15-β, 5 항목)
...
- **C4 (Cross-share enabler)**: M4 LayerClassifier (visual_attn_ratio HOT/COLD/MEDIUM 자동 분류) 가 Bivouac / Strata / Harbinger 의 layer-class 입력으로 cross-share — 본 세션 6 idea 중 4 idea 의 enabler
...
### 5.2 🥈 Bivouac — contribution bullet (5 항목)
...
### 5.3 🥉 RadixVL — contribution bullet (5 항목)
...
- **C4 (Cross-share with Prism)**: ATRIUM 의 M1 LayerClassifier (HOT/COLD visual_attn_ratio 자동 분류) 가 본 세션 Prism M4 LayerClassifier 와 cross-share dependency — 동일 visual_attn_ratio measurement infrastructure 공유 (`tools/atrium_calibrate.py::measure_layer_attn`). 학생이 ATRIUM prototype 후 Prism 의 NVFP4 mixed precision routing 을 추가 적용 가능 — sequential development path
...
| Rank | Title | Score | 5-axis | 링크 |
|------|-------|-------|--------|------|
| T1 | Strata | nov 6.5 / diff 7.5 / impact 7.0 | Performance -15-22% latency / Memory eff. +40-55% / Energy -10-15% | [tier2/01-strata.md](tier2/01-strata.md) |
| T2 | Harbinger | nov 7.0 / diff 7.5 / impact 6.5 | decode +32% / Energy -28% / Peak power -18% | [tier2/02-harbinger.md](tier2/02-harbinger.md) |
| T3 | Obelisk | nov 5.5 / diff 6.5 / impact 7.5 | decode +24% / Power -13% / TTFT -15% | [tier2/03-obelisk.md](tier2/03-obelisk.md) |

### 6.1 T1 Strata — contribution bullet (4 항목)
...
### 6.2 T2 Harbinger — contribution bullet (4 항목)
...
### 6.3 T3 Obelisk — contribution bullet (4 항목)
...
- **TIDEGATE** (UMA bandwidth gating + L2 carveout duty cycle + nvpmodel hopper) — Nova + PD-Multiplexing 와 80%+ overlap, R56.2 30% 미달, gain ceiling 작음. drop, M3 nvpmodel hopper 일부는 Prism M3 KSF-VCD 로 흡수
...
- **cluster** (본 세션 의미) — **K-means visual KV cluster centroid** (Bivouac) 또는 **HOT/COLD layer class** (Prism LayerClassifier). Datacenter cluster (multi-node) 와 무관
- **scale** (본 세션 의미) — **NVFP4 quantization scale** (per-block FP8 + per-tensor FP32) 또는 **scale-freeze (KSF-VCD M3)**. SI unit scale 과 무관
- **tier** (본 세션 의미) — **Strata 의 memory tier** (L2 / GDDR7|UMA / page-color cold) 또는 **본 세션 idea Tier-1/Tier-2** (venue priority). Container tier 와 무관

### 9.3 Idea Metaphor Noun → Mechanism 대응

- **Prism** — 빛 (NVFP4 precision) 이 layer 별로 differential 로 분광 (mixed precision) + visual context fog (DVFS slip) 가 idle decode step 에 GPU clock 을 낮춤 + FX (FleX tensor-core) 가 4:8 sparse 와 dense 를 layer 마다 다른 itinerary 로 dispatch
...
- **RadixVL** — visual prefix 의 LSH semantic hash 가 RadixAttention prism 처럼 분광되어 second-level branch
- **Strata** — KV layout 이 stratified (지층, L2 / GDDR7|UMA / page-color cold) layer 별 매핑
- **Harbinger** — visual confidence harbinger (전조) 가 speculative draft length + cluster heat (4-leaf clover lucky pattern) 가 early-exit
- **Obelisk** — RTX 5090 single-GPU 가 large MoE obelisk (단독 monument, 거대한 inference stack 의 monolith) 로 local serving
