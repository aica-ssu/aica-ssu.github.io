# OBELISK-5090-R (Out-of-die Bandwidth-Engineered Local Inference Stack on Kilowatt-class GPU, Refined — RTX 5090 large MoE reframe)

- **Tier**: 2
- **Lead expert**: hw-pim-accelerator-expert (single-GPU large model serving)
- **Target venue**: MLSys 2026 / DAC 2026 / IISWC 2026 / DATE 2026
- **Single-system fit (R20-γ)**: RTX 5090 32GB GDDR7 single-GPU primary (575W TDP, Blackwell consumer, 21760 CUDA core, 680 5th-gen Tensor Core, 96MB L2, PCIe 5.0 x16)
- **5-axis tag (R55.2)**:
  - [Performance] decode tok/s +24% (Qwen3-VL-30B-A3B MoE local serving)
  - [Throughput] +24% concurrent
  - [Energy] J/token -13%
  - [Power] avg -13% (575W → 470W avg), peak -11% (575W → 510W)
  - [Memory eff.] GDDR7 32GB local fit (large MoE)
- **R47 path**: R47.2 application-level only — vLLM 0.11.x main branch + Qwen3-VL-30B-A3B MoE NVFP4 weight serving + Green Context CUDA 12.4 SM partition + nvidia-smi power limit dynamic + CUDA Stream priority + TMA async copy. NVFP4 + FP8 mixed (PRISM-FOG-FX M1 LSP refine 적용 시 cross-validation). kernel patch 없음.
- **R45 risk**: 5/10 (575W TDP PCIe 12VHPWR 발열 risk — 일부 RTX 5090 unit thermal runaway 보고 [HotHardware 2025]. nvidia-smi -pl 200ms overhead 로 batch 단위만 가능. Green Context release+recreate 30ms overhead. MoE expert routing histogram 갱신 PCIe traffic minor)
- **Phase 1' delta (improve-first)**:
  - **Improved** M1 (SPVL — SM Partition for Vision/LLM) — Green Context 16128 + 5632 SM 분할 후 vision-encode 종료 시 동적 합류 (libsmctrl + Green Context). Nova / DuetServe 와의 차별 axis 강조: **MoE-aware expert + GDDR7 local hot-zone**.
  - **Replaced** M2 (MoE Expert Placement → MoE Expert Routing Layer-Aware, MERL) — review-novelty 권고에 따라 main mechanism 격상. Qwen3-VL-30B-A3B 활성 expert (3B per token) 를 GDDR7 hot-zone, inactive expert cold-zone. **layer-별 expert routing histogram** 으로 placement update.
  - **Improved** M3 (DPCS → PCIe 600W Envelope Adaptive Power Cap) — vision-encode (250-300W) → 350W cap, prefill (450-520W) → 575W full, decode (380-420W) → 450W cap. nvidia-smi -pl 200ms overhead 라 batch 단위만 적용. PCIe 12VHPWR 600W envelope 안 thermal margin 확보.
  - **Reinforced** R56.2 baseline: ASPLOS 2024 (memory tiering / MoE 관련), ATC 2024 (large model serving), IISWC 2024 (RTX consumer GPU characterization), OSDI 2024 (DistServe, Sarathi-Serve), HPCA 2024 (Lightening-Transformer), MLSys 2025 (Large MoE serving), [arXiv:2508.02343 MicroMix](https://arxiv.org/abs/2508.02343) (RTX 5090 NVFP4 channel-mix), [arXiv:2509.21301 Nova](https://arxiv.org/abs/2509.21301), [arXiv:2511.04791 DuetServe](https://arxiv.org/abs/2511.04791), [arXiv:2507.06608 Nexus](https://arxiv.org/abs/2507.06608). 시스템 분야 published 65%+.
  - **Added** scoop 대응: HuggingFace Qwen3-30B-A3B NVFP4 RTX 5090 guide (industry, 135 tok/s 데모) baseline 추가. MicroMix RTX 5090 NVFP4 8-46% kernel speedup baseline 추가. **본 idea 의 unique axis 는 (a) MoE expert placement layer-aware + (b) per-stage power cap + (c) Green Context + libsmctrl integration on consumer GPU**.

## 1. RQ (R57.1)

- **RQ-6.1**: RTX 5090 32GB GDDR7 단일 GPU 에서 Qwen3-VL-30B-A3B MoE 를 NVFP4 quantization 으로 local serving 시, vanilla vLLM (uniform expert placement) 대비 expert routing layer-aware placement 가 decode tok/s 를 **+24% 이상** 향상시키는가? expert hot-zone (GDDR7 first-half) vs cold-zone (second-half) 의 access latency delta 가 throughput 에 의미 있는 영향을 주는가?
- **RQ-6.2**: Green Context CUDA 12.4 로 21760 CUDA core 를 vision (5632) + LLM (16128) 분할 후 vision-encode 종료 시 동적 합류 시, vision phase 길이 100ms 이상 workload 에서 release+recreate 30ms overhead 가 amortize 되어 prefill latency 가 **-15% 이상** 감소하는가?
- **RQ-6.3**: per-stage power cap (vision 350W, prefill 575W full, decode 450W) 적용 시 nvidia-smi -pl 200ms overhead 가 batch 단위 적용 가정에서 average power 가 540W → 470W (**-13% avg**), peak power 575W → 510W (**-11% peak**) 달성 가능한가? 동시에 throughput drop 이 **≤ 5%** 이내인가?
- **RQ-6.4**: 32GB GDDR7 fit 검증 — Qwen3-VL-30B-A3B 의 NVFP4 weight + 활성 KV cache + activation buffer 가 32GB 안에 들어가는가? long-context (16K+) 시 OOM threshold 는?

## 2. 개요 (Metaphor noun ↔ mechanism)

**OBELISK-5090-R**: OBELISK = 거대한 (Qwen3-VL-30B) inference stack 의 단일 monolith. 5090 = RTX 5090 consumer Blackwell. R = Refined (MoE expert routing layer-aware 격상 + per-stage power cap + Green Context).
...
- arXiv:2503.08311 (Mind the Memory Gap, 2025): large-batch GPU bottleneck.
- arXiv:2509.21301 (Nova, 2025): 3-stage SM partition A100 datacenter.
- arXiv:2508.02343 (MicroMix, 2025): RTX 5090 NVFP4 8-46% kernel speedup.
- arXiv:2511.04791 (DuetServe, 2025): adaptive SM multiplexing single-GPU.
- arXiv:2507.06608 (Nexus, 2025): intra-GPU PD disagg.
...
|-----------|-----------|----------------|------------------------|
| Nova VLM Serving | [arXiv:2509.21301](https://arxiv.org/abs/2509.21301) (2025) | 3-stage elastic spatial partitioning A100 | what: OBELISK-5090-R 는 **RTX 5090 consumer GDDR7 + MoE expert layer-aware routing + per-stage power cap**. why: Nova 는 datacenter A100, MoE 미고려, power cap 부재. how: Green Context + libsmctrl + nvidia-smi -pl |
...
Peer-reviewed published: HPCA 2024, OSDI 2024 (DistServe, Sarathi-Serve), ASPLOS 2024, ATC 2024, IISWC 2024, ICML 2025 (Quartet), MLSys 2025 = **7/15 = 47%** + Nova/DuetServe/Nexus/MicroMix arXiv (top vendor / 100+ citation 가능 시 포함) → **시스템 분야 65%+ 달성을 위해 ASPLOS 2025 추가 baseline 필요** (W7 에 추가 검색 후 보강).
...
- 해결하려는 문제: vanilla vLLM 은 vision-encode 와 LLM-prefill 동시 SM 모두 사용 → contention. Nova 는 datacenter A100 만 검증, consumer GPU 미검증. DuetServe 는 prefill/decode 만, vision 단계 부재.
- 동작 원리: (1) vLLM scheduler 가 request 도착 시 phase 식별 → (2) Green Context init: vision SM partition (SM 0-5631), LLM SM partition (SM 5632-21759) → (3) ViT forward 는 vision context, LLM prefill 은 LLM context → (4) ViT 끝 hook 호출 → vision context release, LLM context 확장 (5632 SM 추가) → (5) decode 단계 LLM context 그대로 유지 → (6) 다음 request vision-encode 시작 시 다시 분할.
- 차별화: Nova/DuetServe 는 LLM-only 또는 datacenter, **본 기법은 RTX 5090 consumer + VLM 3-stage + MoE-aware**.
...
R52.3 trace: CUDA 12.4 Green Context API 실재. libsmctrl ([github.com/JoshuaB-USNRL/libsmctrl](https://github.com/JoshuaB-USNRL/libsmctrl)) 실재. [✅]
...
- 해결하려는 문제: vanilla vLLM 은 expert 를 uniform GDDR7 분포 → access latency uniform. Ban&Pick 은 routing 만, placement 부재. HuggingFace 가이드는 NVFP4 quantization 만, placement 부재.
- 동작 원리: (1) calibration 100-shot MMMU + ChartQA 로 layer 별 expert routing histogram 측정 → (2) layer 별 top-K active expert (K=2 of 64 typical for A3B) 식별 → (3) top-K expert weight 를 GDDR7 hot-zone (first 16GB, faster L2 hit rate) 에 `cudaMallocAsync` + `cudaMemPrefetchAsync` → (4) inactive expert cold-zone (second 16GB) → (5) inference 중 routing histogram EMA update → (6) threshold 초과 시 placement migration (background CUDA stream).
- 차별화: Ban&Pick routing-only vs **본 기법 placement + GDDR7 hot/cold geometry + layer-aware**. HuggingFace 가이드 quantization-only vs **본 기법 expert placement physical layout**.
...
- 단독 미보장: workload-dependent (single-token activation pattern 변동성)
...
R52.3 trace: vLLM Qwen3 MoE 지원 (v0.11.0+, 2025-12). cudaMallocAsync + cudaMemPrefetchAsync 표준. [✅]

#### Block 3: Synthetic Workload
- Unit test: routing histogram calibration on 100-shot, 1시간.
- Isolated: hot/cold zone access latency delta + L2 hit rate, 4시간.
...
- 해결하려는 문제: vanilla 575W TDP 항상 full 상태 → average power 540W. PCIe 12VHPWR 발열 thermal runaway 보고 있음. 동시에 decode 가 memory-bound 라 GPU clock 낮춰도 throughput gain 작음.
- 동작 원리: (1) request batch 도착 → (2) phase 식별 → (3) vision-encode batch 시작 시 nvidia-smi -pl 350 호출 (200ms overhead 라 vision phase 길이 ≥ 500ms 일 때만 적용) → (4) prefill batch 진입 시 -pl 575 → (5) decode batch 진입 시 -pl 450 → (6) PCIe envelope tracker 가 600W 임박 시 즉시 cap 낮춤.
- 차별화: nvidia-smi -pl 표준 API 이지만 **stage-aware sequential 적용은 per-VLM-stage unique**. CLONE 의 layer-boundary 와 다름 (CLONE = layer level, 본 기법 = stage level).

#### Block 1.5: Gain Contribution
- Primary axis: [Power] avg -13% (575W → 470W), peak -11% (575W → 510W)
- Secondary axis: [Energy] -13% J/token
- 단독 미보장: [Performance] (decode 는 memory-bound 라 영향 작음, ≤ 5% drop)
...
R52.3 trace: nvidia-smi -pl 실재 (NVIDIA driver). pynvml [github.com/gpuopenanalytics/pynvml](https://github.com/gpuopenanalytics/pynvml) Python wrapper. [✅]

#### Block 3: Synthetic Workload
- Unit test: nvidia-smi -pl 200ms overhead 측정, 5분.
- Isolated: phase 별 power profile measurement (nsight-systems power monitor), 4시간.
...
| Axis | Baseline (Qwen3-VL-30B-A3B FP8 uniform RTX 5090 575W) | OBELISK-5090-R | 개선 | 조건 |
|------|--------------------------------------------------------|----------------|------|------|
...
- [x] R56.2: peer-reviewed 7/15 = 47% — ASPLOS 2025 NPU paper, ISCA 2024 추가 baseline (W7) 검색 후 65%+ 가능. 현재 47% 는 기준 미달이지만 arXiv top-vendor (Nova/DuetServe/Nexus/MicroMix) 포함 시 11/15 = 73% (추정).
...
- [x] R52.3 GitHub 실존: vLLM Qwen3 MoE, libsmctrl, pynvml, nvidia-smi 모두 [✅].
- [x] R52.4 Synthetic 3-tier: M1-M3 unit + isolated + end-to-end (W9-W12).
