...
| 수치 반올림 | 소수점 한 자리 통일 (두 자리는 과도, 정수는 부족) |

### 새로 추가된 원칙 (v2.1)

| 원칙 | 설명 |
|------|------|
| 열거 후 구조화 | "two strategies" 후 반드시 First/Second |
| Intro 섹션 참조 금지 | "(Section 3.1)" 형태의 forward reference 금지 |
| 대조 도입 | 연속 개념 설명 시 "Unlike A, B..." 패턴 |
| 일회성 용어 금지 | 논문에 한 번만 등장하는 신조어 금지 |
| Eval figure 4단계 분석 | baseline 경향 → 제안 경향 → 비교 → 특정 케이스 |
| "weight" 동사 금지 | AI/ML에서 weight는 명사로만 ("scales by" 등 대체) |
| 문단 마무리 과잉 금지 | 데이터가 보여주는 것까지만 기술 |
| "up to" vs 범위 | 전체 일관 개선이면 범위, 특정 케이스만이면 up to |
| 동일 단어 다른 의미 금지 | 같은 단어로 다른 개념 지칭 금지 |
| 전치사 통일 | at/with/for 등 설정값 전치사 일관 |
| Sentence opener 변주 | "Prior work" 변주는 OK (핵심 용어 동의어와 다름) |
| Conclusion = 종합 | 재작성/재설명 아닌 간결한 종합, 1-2문단 |
| 비교 구조 대칭 | 비교 대상의 설명 구조(결과→이유)를 통일 |
...
| 42-53 | **신규** 지도 패턴 원칙 | 열거 구조화, Intro 참조 금지, 대조 도입, figure 4단계 분석, 문단 마무리 절제 등 |
